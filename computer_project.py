...
def create_dictionaries(file_content: list[str]) -> tuple[dict, dict]:
    vertical_out = {}
    vertical_in = {}
    page_rank = {}
    vertexes = set()
    ...
    for elem in file_content[1:-1]:
        ...
        vertexes.update(start_point, end_point)
    ...
    page_rank = page_rank.fromkeys(vertexes, 1/(len(vertexes) + len(not_used_vertexes)))
    page_rank.update(page_rank.fromkeys(not_used_vertexes, (1 - 0.85)/(len(vertexes) + len(not_used_vertexes))))
    page_rank = dict(sorted(page_rank.items(), key=lambda x: x[0]))

    # print(f'{vertical_out} - вихід: вхід')
    # print(f'{vertical_in} - вхід: вихід')
    # print(f'{sorted(vertexes)} - вершини звязні')
    # print(f'{sorted(not_used_vertexes)} - вершини ізольовані')
    # print(page_rank)
...
def get_page_rank(page_rank, out_in_ribs, in_out_ribs, peaks, useless_peaks):
    all_vertexes_counter = len(useless_peaks) + len(peaks)
    ...
    while start or any(abs(previous_pr[value] - page_rank[value]) > 1e-6 for value in page_rank):
        ...
        for peak in peaks:
            coefficients_sum = 0
            if peak in in_out_ribs.keys(): #Якщо вершина має вхід
                for _, point in enumerate(in_out_ribs[peak]):
                    point_rank = page_rank[point]
